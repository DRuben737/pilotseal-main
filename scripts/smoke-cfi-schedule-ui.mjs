...
try {
  ...
  assert.equal(await navigationToggle.getAttribute('aria-expanded'),'false','site navigation starts collapsed');
  ...
  assert.equal(await page.getByRole('button',{name:'Close navigation',exact:true}).getAttribute('aria-expanded'),'true','hamburger opens site navigation');
  await page.getByRole('button',{name:'Close navigation',exact:true}).click();
  const dashboardNavigation=page.getByRole('navigation',{name:'Dashboard navigation',exact:true});
  assert(await dashboardNavigation.isVisible(),'personal functions stay directly available in the compact mobile strip');
  await page.screenshot({path:path.join(screenshots,'compact-functions-390.png'),fullPage:true});
  await dashboardNavigation.getByRole('link',{name:'Schedule',exact:true}).click();
  ...
  assert(await page.evaluate(()=>Boolean(document.elementFromPoint(innerWidth/2,innerHeight-10)?.closest('[role="dialog"]'))),'block editor stays above mobile navigation');
  ...
  const autoDialog=page.getByRole('dialog',{name:'Automatic scheduling',exact:true});
  const autoStudent=autoDialog.getByRole('checkbox',{name:/UI Schedule Student/});
  const autoCount=autoDialog.getByLabel('Lessons to add for UI Schedule Student',{exact:true});
  assert(await autoStudent.isChecked(),'students below their weekly goal are selected by default');
  assert.equal(await autoCount.inputValue(),'3','the default adds enough lessons to reach the weekly goal');
  await autoDialog.getByRole('button',{name:'Clear',exact:true}).click();
  assert(await autoDialog.getByRole('button',{name:'Generate preview',exact:true}).isDisabled(),'generation requires an explicit selection');
  await autoDialog.getByRole('button',{name:'Select all',exact:true}).click();
  await autoCount.fill('1');
  assert.equal(await autoCount.inputValue(),'1','the instructor controls the exact additions for each student');
  assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'automatic scheduling roster does not overflow the page');
  await page.screenshot({path:path.join(screenshots,'automatic-roster-390.png'),fullPage:true});
  await autoDialog.getByRole('button',{name:'Generate preview',exact:true}).click();
  await autoDialog.getByText('1 of 1 requested Flight lessons fit.',{exact:true}).waitFor();
  await autoDialog.getByText('1/1 scheduled',{exact:true}).waitFor();
  await autoDialog.getByRole('button',{name:'Add 1 Flight lesson to draft',exact:true}).click();
