-- Logical links between an account owner's private saved-person record and a
-- different verified PilotSeal account. Apply through the versioned migration;
-- this file is the readable canonical definition for future schema work.

create table if not exists public.saved_person_account_links (
  saved_person_id uuid primary key references public.saved_people(id) on delete cascade,
  owner_user_id uuid not null references auth.users(id) on delete cascade,
  linked_user_id uuid not null references public.profiles(id) on delete cascade,
  linked_at timestamptz not null default timezone('utc', now()),
  constraint saved_person_account_links_not_self_check check (owner_user_id <> linked_user_id),
  constraint saved_person_account_links_owner_linked_key unique (owner_user_id, linked_user_id)
);
...
drop policy if exists saved_person_account_links_select_own on public.saved_person_account_links;
...
create or replace function public.link_saved_person_account(p_saved_person_id uuid, p_email text)
...
begin
  ...
  where id = p_saved_person_id and user_id = v_caller_id
  for update;
  ...
  values (v_person.id, v_caller_id, v_linked_user_id)
...
as $$
  ...
  where auth.uid() is not null
    and links.owner_user_id = auth.uid()
  group by links.saved_person_id, links.owner_user_id, links.linked_user_id, links.linked_at
...
revoke all on function public.unlink_saved_person_account(uuid) from public, anon, authenticated;
revoke all on function public.list_my_saved_person_account_links() from public, anon, authenticated;
grant execute on function public.link_saved_person_account(uuid, text) to authenticated;
grant execute on function public.unlink_saved_person_account(uuid) to authenticated;
grant execute on function public.list_my_saved_person_account_links() to authenticated;
