    ...
    priorities: {
      ...
      items: {
        ...
        properties: {
          ...
          possible_consequences: { type: "string", minLength: 1, maxLength: 240 },
          compounding_effect: { type: "string", minLength: 1, maxLength: 200 },
          mitigations: { type: "array", minItems: 1, maxItems: 2, items: { type: "string", minLength: 1, maxLength: 160 } },
          recheck_triggers: { type: "array", minItems: 1, maxItems: 2, items: { type: "string", minLength: 1, maxLength: 160 } },
        },
      },
    },
    residual_risk: { type: "string", minLength: 1, maxLength: 240 },
  },
};
...
export function validateRiskDiscussionOutput(value, allowedEvidenceIds) {
  ...
  const overview = assertText(value.overview, "overview", 320);
  const residualRisk = assertText(value.residual_risk, "residual risk", 240);
  if (!Array.isArray(value.priorities) || value.priorities.length > 3) throw new Error("AI response has invalid priorities.");
  ...
  const priorities = value.priorities.map((item, index) => {
    ...
    const list = (key) => {
      if (!Array.isArray(item[key]) || item[key].length < 1 || item[key].length > 2) throw new Error(`Invalid ${key}.`);
      return item[key].map((entry) => assertText(entry, key, 160));
    };
    return {
      title: assertText(item.title, "priority title", 90),
      evidenceIds: [...new Set(item.evidence_ids)],
      possibleConsequences: assertText(item.possible_consequences, "possible consequences", 240),
      compoundingEffect: assertText(item.compounding_effect, "compounding effect", 200),
